...
const Tooltip = ({ text, size }: Props) => {
  ...
  return (
    ...
    >
      ...
      {showTooltip && (
        <div
          className={`w-[150px] absolute top-0 ${size === "small" ? "left-[15px]" : "left-[25px]"} bg-white game-list-bg border-blue-800 border-[2px] rounded-[16px] p-[6px] z-50`}
        >
          <p className={`${size === "small" ? "text-xs" : "text-sm"}  text-white`}>{text}</p>
        </div>
      )}
    </button>
  );
