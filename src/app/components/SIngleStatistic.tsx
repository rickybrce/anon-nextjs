...
const SingleStatistic = ({ index, title, value, image }: Props) => {
    ...
    return (
        <div className={`relative border-green-100 ${(index === 0 || index === 1 || index === 2) && "border-b-[3px]"}`}>
            <div className={`relative flex flex-wrap justify-between border-green-100 my-[30px] py-[20px] px-[50px] ${(index !== 2 && index !== 5 && index !== 8) && "border-r-[3px]"}`}>
                <div className=''>
                    <div className='font-w-700 text-[20px] leading-[28px] text-white mb-1'>{title}</div>
